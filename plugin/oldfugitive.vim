" oldfugitive.vim - A Git wrapper so awesome, it should be illegal
" Maintainer:   Tim Pope <http://tpo.pe/>
" Version:      2.2
" GetLatestVimScripts: 2975 1 :AutoInstall: oldfugitive.vim

if exists('g:loaded_oldfugitive') || &cp
  finish
endif
let g:loaded_oldfugitive = 1

if !exists('g:oldfugitive_git_executable')
  let g:oldfugitive_git_executable = 'git'
endif
...
function! s:throw(string) abort
  let v:errmsg = 'oldfugitive: '.a:string
  throw v:errmsg
endfunction
...
function! s:git_command() abort
  return get(g:, 'oldfugitive_git_command', g:oldfugitive_git_executable)
endfunction

function! oldfugitive#git_version(...) abort
  if !has_key(s:git_versions, g:oldfugitive_git_executable)
    let s:git_versions[g:oldfugitive_git_executable] = matchstr(system(g:oldfugitive_git_executable.' --version'), "\\S\\+\n")
  endif
  return s:git_versions[g:oldfugitive_git_executable]
endfunction
...
function! oldfugitive#is_git_dir(path) abort
...
function! oldfugitive#extract_git_dir(path) abort
  if s:shellslash(a:path) =~# '^oldfugitive://.*//'
    return matchstr(s:shellslash(a:path), '\C^oldfugitive://\zs.\{-\}\ze//')
  endif
  ...
  while root !=# previous
    ...
    if root ==# $GIT_WORK_TREE && oldfugitive#is_git_dir($GIT_DIR)
      return simplify(fnamemodify(expand($GIT_DIR), ':p:s?[\/]$??'))
    endif
    if oldfugitive#is_git_dir($GIT_DIR)
    ...
    if type ==# 'dir' && oldfugitive#is_git_dir(dir)
      return dir
    elseif type ==# 'link' && oldfugitive#is_git_dir(dir)
      return resolve(dir)
    elseif type !=# '' && filereadable(dir)
      let line = get(readfile(dir, '', 1), 0, '')
      if line =~# '^gitdir: \.' && oldfugitive#is_git_dir(root.'/'.line[8:-1])
        return simplify(root.'/'.line[8:-1])
      elseif line =~# '^gitdir: ' && oldfugitive#is_git_dir(line[8:-1])
        return line[8:-1]
      endif
    elseif oldfugitive#is_git_dir(root)
      return root
    endif
    let previous = root
...
function! oldfugitive#detect(path) abort
  if exists('b:git_dir') && (b:git_dir ==# '' || b:git_dir =~# '/$')
    unlet b:git_dir
  endif
  if !exists('b:git_dir')
    let dir = oldfugitive#extract_git_dir(a:path)
    if dir !=# ''
      let b:git_dir = dir
      if empty(oldfugitive#buffer().path())
        silent! exe haslocaldir() ? 'lcd .' : 'cd .'
      endif
    endif
  endif
  if exists('b:git_dir')
    if exists('#User#OldFugitiveBoot')
      try
        let [save_mls, &modelines] = [&mls, 0]
        doautocmd User OldFugitiveBoot
      finally
        let &mls = save_mls
      endtry
    endif
    if !exists('g:oldfugitive_no_maps')
      cnoremap <buffer> <expr> <C-R><C-G> fnameescape(<SID>recall())
      nnoremap <buffer> <silent> y<C-G> :call setreg(v:register, <SID>recall())<CR>
    endif
    let buffer = oldfugitive#buffer()
    if expand('%:p') =~# '://'
...
augroup oldfugitive
  autocmd!
  autocmd BufNewFile,BufReadPost * call oldfugitive#detect(expand('%:p'))
  autocmd FileType           netrw call oldfugitive#detect(expand('%:p'))
  autocmd User NERDTreeInit,NERDTreeNewRoot call oldfugitive#detect(b:NERDTree.root.path.str())
  autocmd VimEnter * if expand('<amatch>')==''|call oldfugitive#detect(getcwd())|endif
  autocmd CmdWinEnter * call oldfugitive#detect(expand('#:p'))
  autocmd BufWinLeave * execute getwinvar(+bufwinnr(+expand('<abuf>')), 'oldfugitive_leave')
augroup END
...
function! s:repo(...) abort
  let dir = a:0 ? a:1 : (exists('b:git_dir') && b:git_dir !=# '' ? b:git_dir : oldfugitive#extract_git_dir(expand('%:p')))
  if dir !=# ''
...
function! oldfugitive#repo(...) abort
...
function! s:repo_translate(spec) dict abort
  ...
  elseif a:spec =~# '^:[0-3]:'
    return 'oldfugitive://'.self.dir().'//'.a:spec[1].'/'.a:spec[3:-1]
  elseif a:spec ==# ':'
  ...
  elseif a:spec =~# '^:/'
    ...
    return 'oldfugitive://'.self.dir().'//'.ref
  elseif a:spec =~# '^:'
    return 'oldfugitive://'.self.dir().'//0/'.a:spec[1:-1]
  elseif a:spec ==# '@'
  ...
  elseif filereadable(refs.'remotes/'.a:spec.'/HEAD')
    ...
    try
      ...
      return 'oldfugitive://'.self.dir().'//'.ref.path
    catch /^oldfugitive:/
      return self.tree(a:spec)
    endtry
  endif
...
function! s:repo_git_chomp(...) dict abort
  let git = g:oldfugitive_git_executable . ' --git-dir='.s:shellesc(self.git_dir)
...
function! oldfugitive#buffer(...) abort
...
function! s:buffer_type(...) dict abort
  if self.getvar('oldfugitive_type') != ''
    let type = self.getvar('oldfugitive_type')
...
function! s:buffer_commit() dict abort
  return matchstr(self.spec(),'^oldfugitive://.\{-\}//\zs\w*')
endfunction
...
function! s:buffer_path(...) dict abort
  let rev = matchstr(self.spec(),'^oldfugitive://.\{-\}//\zs.*')
  if rev != ''
    let rev = s:sub(rev,'\w*','')
...
function! s:buffer_sha1() dict abort
  if self.spec() =~ '^oldfugitive://' || self.spec() =~ '\.git/refs/\|\.git/.*HEAD$'
...
function! s:Git(bang, args) abort
  ...
  if exists(':terminal') && has('nvim')
    ...
    if has('win32')
      call oldfugitive#reload_status()
    endif
  endif
  return matchstr(a:args, '\v\C\\@<!%(\\\\)*\|\zs.*')
endfunction

function! oldfugitive#git_commands() abort
  if !exists('s:exec_path')
    let s:exec_path = s:sub(system(g:oldfugitive_git_executable.' --exec-path'),'\n$','')
  endif
...
function! s:GitComplete(A, L, P) abort
  if strpart(a:L, 0, a:P) !~# ' [[:alnum:]-]\+ '
    let cmds = oldfugitive#git_commands()
...
augroup oldfugitive_status
  autocmd!
  if !has('win32')
    autocmd FocusGained,ShellCmdPost * call oldfugitive#reload_status()
    autocmd BufDelete term://* call oldfugitive#reload_status()
  endif
augroup END
...
function! oldfugitive#reload_status() abort
  ...
  try
    ...
    for tab in [mytab] + range(1,tabpagenr('$'))
      for winnr in range(1,tabpagewinnr(tab,'$'))
        if getbufvar(tabpagebuflist(tab)[winnr-1],'oldfugitive_type') ==# 'index'
          execute 'tabnext '.tab
          if winnr != winnr()
  ...
  try
    ...
    if !error
      ...
      if error =~# 'false''\=\.$'
        ...
        let b:oldfugitive_commit_arguments = args
...
function! s:FinishCommit() abort
  let args = getbufvar(+expand('<abuf>'),'oldfugitive_commit_arguments')
  if !empty(args)
    call setbufvar(+expand('<abuf>'),'oldfugitive_commit_arguments','')
...
function! oldfugitive#cwindow() abort
  if &buftype == 'quickfix'
    cwindow
  else
    botright cwindow
...
function! s:Merge(cmd, bang, args) abort
  ...
  try
    ...
    if a:cmd =~# '^merge' && empty(a:args) &&
      ...
      let &l:makeprg = g:oldfugitive_git_executable.' diff-files --name-status --diff-filter=U'
    else
...
if !exists('g:oldfugitive_summary_format')
  let g:oldfugitive_summary_format = '%s'
endif
...
function! s:Log(cmd, line1, line2, ...) abort
  ...
  let cmd += ['--pretty=format:oldfugitive://'.s:repo().dir().'//%H'.path.'::'.g:oldfugitive_summary_format]
...
function! s:Edit(cmd,bang,...) abort
  let buffer = s:buffer()
  if a:cmd !~# 'read'
    if &previewwindow && getbufvar('','oldfugitive_type') ==# 'index'
      if winnr('$') == 1
...
function! s:Write(force,...) abort
  if exists('b:oldfugitive_commit_arguments')
  ...
  if empty(path)
    return 'echoerr '.string('oldfugitive: cannot determine file path')
  endif
  if path =~# '^:\d\>'
  ...
  if !always_permitted && !a:force && s:repo().git_chomp_in_tree('diff','--name-status','HEAD','--',path) . s:repo().git_chomp_in_tree('ls-files','--others','--',path) !=# ''
    let v:errmsg = 'oldfugitive: file has uncommitted changes (use ! to override)'
    return 'echoerr v:errmsg'
  endif
  ...
  if v:shell_error
    let v:errmsg = 'oldfugitive: '.error
    return 'echoerr v:errmsg'
  endif
...
function! s:Dispatch(bang, args)
  ...
  try
    ...
    if exists(':Make') == 2
      ...
      silent noautocmd make!
      redraw!
      return 'call oldfugitive#cwindow()'
    endif
    return ''
  finally
...
function! s:can_diffoff(buf) abort
  return getwinvar(bufwinnr(a:buf), '&diff') &&
        ...
        \ !empty(getwinvar(bufwinnr(a:buf), 'oldfugitive_diff_restore'))
endfunction

function! oldfugitive#can_diffoff(buf) abort
...
function! s:diffthis() abort
  if !&diff
    let w:oldfugitive_diff_restore = s:diff_restore()
    diffthis
  endif
endfunction

function! s:diffoff() abort
  if exists('w:oldfugitive_diff_restore')
    execute w:oldfugitive_diff_restore
    unlet w:oldfugitive_diff_restore
  else
    diffoff
  endif
endfunction
...
function! s:Diff(vert,keepfocus,...) abort
  ...
  elseif (empty(args) || args[0] == ':') && s:buffer().commit() =~# '^[0-1]\=$' && s:repo().git_chomp_in_tree('ls-files', '--unmerged', '--', s:buffer().path()) !=# ''
    ...
    let nr = bufnr('')
    execute 'leftabove '.vert.'split' s:fnameescape(oldfugitive#repo().translate(s:buffer().expand(':2')))
    execute 'nnoremap <buffer> <silent> dp :diffput '.nr.'<Bar>diffupdate<CR>'
    let nr2 = bufnr('')
    call s:diffthis()
    wincmd p
    execute 'rightbelow '.vert.'split' s:fnameescape(oldfugitive#repo().translate(s:buffer().expand(':3')))
    execute 'nnoremap <buffer> <silent> dp :diffput '.nr.'<Bar>diffupdate<CR>'
    let nr3 = bufnr('')
    call s:diffthis()
    wincmd p
  ...
  try
    ...
    let w:oldfugitive_diff_restore = restore
...
function! s:Move(force,destination) abort
  ...
  if v:shell_error
    let v:errmsg = 'oldfugitive: '.message
    return 'echoerr v:errmsg'
  endif
...
function! s:Remove(after, force) abort
  ...
  elseif s:buffer().commit() ==# '0'
    ...
    let v:errmsg = 'oldfugitive: rm not supported here'
    return 'echoerr v:errmsg'
  endif
  if a:force
    let cmd += ['--force']
  ...
  if v:shell_error
    let v:errmsg = 'oldfugitive: '.s:sub(message,'error:.*\zs\n\(.*-f.*',' (add ! to force)')
...
augroup oldfugitive_remove
  autocmd!
  autocmd User OldFugitive if s:buffer().commit() =~# '^0\=$' |
...
augroup oldfugitive_blame
  autocmd!
  autocmd BufReadPost *.oldfugitiveblame setfiletype oldfugitiveblame
  autocmd FileType oldfugitiveblame setlocal nomodeline | if exists('b:git_dir') | let &l:keywordprg = s:repo().keywordprg() | endif
  autocmd Syntax oldfugitiveblame call s:BlameSyntax()
  autocmd User OldFugitive if s:buffer().type('file', 'blob') | exe "command! -buffer -bar -bang -range=0 -nargs=* Gblame :execute s:Blame(<bang>0,<line1>,<line2>,<count>,[<f-args>])" | endif
...
function! s:Blame(bang,line1,line2,count,args) abort
  if exists('b:oldfugitive_blamed_bufnr')
    return 'bdelete'
  endif
  try
    if s:buffer().path() == ''
    ...
    try
      ...
      if a:count
        ...
        let temp = error.'.oldfugitiveblame'
        if &shell =~# 'csh'
        ...
        for winnr in range(winnr('$'),1,-1)
          ...
          if getbufvar(winbufnr(winnr), 'oldfugitive_blamed_bufnr')
        ...
        let b:oldfugitive_blamed_bufnr = bufnr
        let w:oldfugitive_leave = restore
        let b:oldfugitive_blame_arguments = join(a:args,' ')
        execute top
        normal! zt
        execute current
        if exists('+cursorbind')
        ...
        setlocal nomodified nomodifiable nonumber scrollbind nowrap foldcolumn=0 nofoldenable winfixwidth filetype=oldfugitiveblame
        ...
        nnoremap <buffer> <silent> <F1> :help oldfugitive-:Gblame<CR>
        nnoremap <buffer> <silent> g?   :help oldfugitive-:Gblame<CR>
        nnoremap <buffer> <silent> q    :exe substitute(bufwinnr(b:oldfugitive_blamed_bufnr).' wincmd w<Bar>'.bufnr('').'bdelete','^-1','','')<CR>
        nnoremap <buffer> <silent> gq   :exe substitute(bufwinnr(b:oldfugitive_blamed_bufnr).' wincmd w<Bar>'.bufnr('').'bdelete<Bar>if expand("%:p") =~# "^oldfugitive:[\\/][\\/]"<Bar>Gedit<Bar>endif','^-1','','')<CR>
...
function! s:BlameCommit(cmd) abort
  ...
  if path ==# ''
    let path = s:buffer(b:oldfugitive_blamed_bufnr).path()
  endif
  execute cmd
...
function! s:BlameJump(suffix) abort
  ...
  let args = b:oldfugitive_blame_arguments
  let offset = line('.') - line('w0')
  let bufnr = bufnr('%')
  let winnr = bufwinnr(b:oldfugitive_blamed_bufnr)
  if winnr > 0
    exe winnr.'wincmd w'
  endif
...
function! s:BlameSyntax() abort
  let b:current_syntax = 'oldfugitiveblame'
  let conceal = has('conceal') ? ' conceal' : ''
  let arg = exists('b:oldfugitive_blame_arguments') ? b:oldfugitive_blame_arguments : ''
  syn match OldFugitiveblameBoundary "^\^"
  syn match OldFugitiveblameBlank                      "^\s\+\s\@=" nextgroup=OldFugitiveblameAnnotation,oldfugitiveblameOriginalFile,OldFugitiveblameOriginalLineNumber skipwhite
  syn match OldFugitiveblameHash       "\%(^\^\=\)\@<=\<\x\{7,40\}\>" nextgroup=OldFugitiveblameAnnotation,OldFugitiveblameOriginalLineNumber,oldfugitiveblameOriginalFile skipwhite
  syn match OldFugitiveblameUncommitted "\%(^\^\=\)\@<=\<0\{7,40\}\>" nextgroup=OldFugitiveblameAnnotation,OldFugitiveblameOriginalLineNumber,oldfugitiveblameOriginalFile skipwhite
  syn region OldFugitiveblameAnnotation matchgroup=OldFugitiveblameDelimiter start="(" end="\%( \d\+\)\@<=)" contained keepend oneline
  syn match OldFugitiveblameTime "[0-9:/+-][0-9:/+ -]*[0-9:/+-]\%( \+\d\+)\)\@=" contained containedin=OldFugitiveblameAnnotation
  exec 'syn match OldFugitiveblameLineNumber         " *\d\+)\@=" contained containedin=OldFugitiveblameAnnotation'.conceal
  exec 'syn match OldFugitiveblameOriginalFile       " \%(\f\+\D\@<=\|\D\@=\f\+\)\%(\%(\s\+\d\+\)\=\s\%((\|\s*\d\+)\)\)\@=" contained nextgroup=OldFugitiveblameOriginalLineNumber,OldFugitiveblameAnnotation skipwhite'.(arg =~# 'f' ? '' : conceal)
  exec 'syn match OldFugitiveblameOriginalLineNumber " *\d\+\%(\s(\)\@=" contained nextgroup=OldFugitiveblameAnnotation skipwhite'.(arg =~# 'n' ? '' : conceal)
  exec 'syn match OldFugitiveblameOriginalLineNumber " *\d\+\%(\s\+\d\+)\)\@=" contained nextgroup=OldFugitiveblameShort skipwhite'.(arg =~# 'n' ? '' : conceal)
  syn match OldFugitiveblameShort              " \d\+)" contained contains=OldFugitiveblameLineNumber
  syn match OldFugitiveblameNotCommittedYet "(\@<=Not Committed Yet\>" contained containedin=OldFugitiveblameAnnotation
  hi def link OldFugitiveblameBoundary           Keyword
  hi def link OldFugitiveblameHash               Identifier
  hi def link OldFugitiveblameUncommitted        Ignore
  hi def link OldFugitiveblameTime               PreProc
  hi def link OldFugitiveblameLineNumber         Number
  hi def link OldFugitiveblameOriginalFile       String
  hi def link OldFugitiveblameOriginalLineNumber Float
  hi def link OldFugitiveblameShort              OldFugitiveblameDelimiter
  hi def link OldFugitiveblameDelimiter          Delimiter
  hi def link OldFugitiveblameNotCommittedYet    Comment
  let seen = {}
  for lnum in range(1, line('$'))
    ...
    exe 'syn match OldFugitiveblameHash'.hash.'       "\%(^\^\=\)\@<='.hash.'\x\{1,34\}\>" nextgroup=OldFugitiveblameAnnotation,OldFugitiveblameOriginalLineNumber,oldfugitiveblameOriginalFile skipwhite'
  endfor
...
function! s:RehighlightBlame() abort
  for [hash, cterm] in items(s:hash_colors)
    if !empty(cterm) || has('gui_running') || has('termguicolors') && &termguicolors
      exe 'hi OldFugitiveblameHash'.hash.' guifg=#'.hash.get(s:hash_colors, hash, '')
    else
      exe 'hi link OldFugitiveblameHash'.hash.' Identifier'
    endif
  endfor
endfunction
...
function! s:Browse(bang,line1,count,...) abort
  try
    ...
    if oldfugitive#git_version() =~# '^[01]\.\|^2\.[0-6]\.'
    ...
    for Handler in g:oldfugitive_browse_handlers
...
function! s:github_url(opts, ...) abort
  ...
  let domains = exists('g:oldfugitive_github_domains') ? g:oldfugitive_github_domains : []
  ...
  if a:opts.commit =~# '^\x\{40\}$'
    ...
    if a:opts.type ==# 'blob' && empty(a:opts.commit)
      ...
      try
        ...
        call s:throw('oldfugitive: cannot browse uncommitted file')
      endtry
    endif
...
if !exists('g:oldfugitive_browse_handlers')
  let g:oldfugitive_browse_handlers = []
endif

call extend(g:oldfugitive_browse_handlers,
...
function! s:BufReadIndex() abort
  if !exists('b:oldfugitive_display_format')
    let b:oldfugitive_display_format = filereadable(expand('%').'.lock')
  endif
  let b:oldfugitive_display_format = b:oldfugitive_display_format % 2
  let b:oldfugitive_type = 'index'
  try
    ...
    if b:oldfugitive_display_format
      call s:ReplaceCmd(s:repo().git_command('ls-files','--stage'),index)
      set ft=git nospell
    else
      let cd = exists('*haslocaldir') && haslocaldir() ? 'lcd' : 'cd'
      let dir = getcwd()
      if oldfugitive#git_version() =~# '^0\|^1\.[1-7]\.'
      ...
      endtry
      set ft=gitcommit
      set foldtext=oldfugitive#foldtext()
    endif
    ...
    nnoremap <buffer> <silent> a :<C-U>let b:oldfugitive_display_format += 1<Bar>exe <SID>BufReadIndex()<CR>
    nnoremap <buffer> <silent> i :<C-U>let b:oldfugitive_display_format -= 1<Bar>exe <SID>BufReadIndex()<CR>
    ...
    nnoremap <buffer> <silent> g?   :help oldfugitive-:Gstatus<CR>
    nnoremap <buffer> <silent> <F1> :help oldfugitive-:Gstatus<CR>
  catch /^oldfugitive:/
    return 'echoerr v:errmsg'
  endtry
endfunction
...
function! s:FileRead() abort
  try
    let repo = s:repo(oldfugitive#extract_git_dir(expand('<amatch>')))
    let path = s:sub(s:sub(matchstr(expand('<amatch>'),'oldfugitive://.\{-\}//\zs.*'),'/',':'),'^\d:',':&')
...
function! s:BufWriteIndexFile() abort
  ...
  try
    ...
    if v:shell_error == 0
      ...
      return 'echoerr '.string('oldfugitive: '.error)
    endif
  finally
    call delete(tmp)
  endtry
...
function! s:BufReadObject() abort
  try
    ...
    if !exists("b:oldfugitive_type")
      let b:oldfugitive_type = s:repo().git_chomp('cat-file','-t',hash)
    endif
    if b:oldfugitive_type !~# '^\%(tag\|commit\|tree\|blob\)$'
      return "echoerr ".string("oldfugitive: unrecognized git type '".b:oldfugitive_type."'")
    endif
    let firstline = getline('.')
    if !exists('b:oldfugitive_display_format') && b:oldfugitive_type != 'blob'
      let b:oldfugitive_display_format = +getbufvar('#','oldfugitive_display_format')
    endif

    if b:oldfugitive_type !=# 'blob'
      setlocal nomodeline
    endif
    ...
    try
      ...
      elseif b:oldfugitive_type ==# 'tag'
        let b:oldfugitive_display_format = b:oldfugitive_display_format % 2
        if b:oldfugitive_display_format
          call s:ReplaceCmd(s:repo().git_command('cat-file',b:oldfugitive_type,hash))
        else
          call s:ReplaceCmd(s:repo().git_command('cat-file','-p',hash))
        endif
      elseif b:oldfugitive_type ==# 'commit'
        let b:oldfugitive_display_format = b:oldfugitive_display_format % 2
        if b:oldfugitive_display_format
          call s:ReplaceCmd(s:repo().git_command('cat-file',b:oldfugitive_type,hash))
        else
      ...
      elseif b:oldfugitive_type ==# 'blob'
        call s:ReplaceCmd(s:repo().git_command('cat-file',b:oldfugitive_type,hash))
        setlocal nomodeline
      endif
    finally
      ...
      if b:oldfugitive_type !=# 'blob'
        ...
        nnoremap <buffer> <silent> a :<C-U>let b:oldfugitive_display_format += v:count1<Bar>exe <SID>BufReadObject()<CR>
        nnoremap <buffer> <silent> i :<C-U>let b:oldfugitive_display_format -= v:count1<Bar>exe <SID>BufReadObject()<CR>
      else
        call s:JumpInit()
      endif
    endtry
...
augroup oldfugitive_files
  autocmd!
  autocmd BufReadCmd  index{,.lock}
        \ if oldfugitive#is_git_dir(expand('<amatch>:p:h')) |
        \   exe s:BufReadIndex() |
        \ elseif filereadable(expand('<amatch>')) |
        \   read <amatch> |
        \   1delete |
        \ endif
  autocmd FileReadCmd oldfugitive://**//[0-3]/**          exe s:FileRead()
  autocmd BufReadCmd  oldfugitive://**//[0-3]/**          exe s:BufReadIndexFile()
  autocmd BufWriteCmd oldfugitive://**//[0-3]/**          exe s:BufWriteIndexFile()
  autocmd BufReadCmd  oldfugitive://**//[0-9a-f][0-9a-f]* exe s:BufReadObject()
  autocmd FileReadCmd oldfugitive://**//[0-9a-f][0-9a-f]* exe s:FileRead()
...
augroup oldfugitive_temp
  ...
  autocmd BufNewFile,BufReadPost *
        ...
        \   call oldfugitive#detect(expand('<afile>:p')) |
...
function! s:GFInit(...) abort
  cnoremap <buffer> <expr> <Plug><cfile> oldfugitive#cfile()
  if !exists('g:oldfugitive_no_maps') && empty(mapcheck('gf', 'n'))
...
function! s:JumpInit(...) abort
  ...
  if !&modifiable
    ...
    nnoremap <buffer> <silent> -     :<C-U>exe <SID>Edit('edit',0,<SID>buffer().up(v:count1))<Bar> if oldfugitive#buffer().type('tree')<Bar>call search('^'.escape(expand('#:t'),'.*[]~\').'/\=$','wc')<Bar>endif<CR>
...
function! oldfugitive#cfile() abort
  let pre = ''
  let results = s:cfile()
  if empty(results)
  ...
  return pre . s:fnameescape(oldfugitive#repo().translate(results[0]))
endfunction
...
function! oldfugitive#statusline(...) abort
  if !exists('b:git_dir')
    return ''
  endif
  let status = ''
...
function! oldfugitive#head(...) abort
  if !exists('b:git_dir')
    return ''
  endif
...
augroup oldfugitive_statusline
  autocmd!
  autocmd User Flags call Hoist('buffer', function('oldfugitive#statusline'))
augroup END

" Section: Folding

function! oldfugitive#foldtext() abort
...
augroup oldfugitive_foldtext
  autocmd!
  autocmd User OldFugitive
